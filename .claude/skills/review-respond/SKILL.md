...
The final markdown reflection uses `{tmp_dir}/events.jsonl`. It is generated
by the aggregator sub-agent, which reads files under `{tmp_dir}/` and emits
them in one pass. Format:
...
2. The leader receives the return value (`{fixed_count, code_changed, summary_line, template_id}`). Verify that `template_id` matches `3b7f1c5d-8a29-4e63-b1c8-9d3a7f5e2b41`; on mismatch, relaunch the sub-agent.
