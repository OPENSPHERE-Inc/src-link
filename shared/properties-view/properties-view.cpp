...
{
    auto shift = [&](unsigned val, int _shift) {
        return ((val & 0xff) << _shift);
    };
...
struct frame_rate_tag {
    ...
    explicit frame_rate_tag(tag_type _type) : type(_type) {}

    explicit frame_rate_tag(const char *_val) : type(USER), val(_val) {}
...
OBSPropertiesView::OBSPropertiesView(
    OBSData settings_, obs_object_t *obj, PropertiesReloadCallback reloadCallback_, PropertiesUpdateCallback callback_,
    PropertiesVisualUpdateCb visUpdateCb_, int minSize_
)
    : VScrollArea(nullptr),
...
OBSPropertiesView::OBSPropertiesView(
    OBSData settings_, void *obj, PropertiesReloadCallback reloadCallback_, PropertiesUpdateCallback callback_,
    PropertiesVisualUpdateCb visUpdateCb_, int minSize_
)
    : VScrollArea(nullptr),
      properties(nullptr, obs_properties_destroy),
...
OBSPropertiesView::OBSPropertiesView(
    OBSData settings_, const char *type_, PropertiesReloadCallback reloadCallback_, int minSize_
)
    : VScrollArea(nullptr),
      properties(nullptr, obs_properties_destroy),
      settings(settings_),
...
QWidget *OBSPropertiesView::NewWidget(obs_property_t *prop, Sender *widget_, void (SenderParent::*signal)(Args...))
{
    const char *long_desc = obs_property_long_description(prop);

    WidgetInfo *info = new WidgetInfo(this, prop, widget_);
    QObject::connect(widget_, signal, info, &WidgetInfo::ControlChanged);
    children.emplace_back(info);

    widget_->setToolTip(QT_UTF8(long_desc));
    return widget_;
}
...
QWidget *OBSPropertiesView::AddCheckbox(obs_property_t *prop)
{
    ...
    QWidget *widget_ = NewWidget(prop, checkbox, &QCheckBox::checkStateChanged);
#else
    QWidget *widget_ = NewWidget(prop, checkbox, &QCheckBox::stateChanged);
#endif
...
QWidget *OBSPropertiesView::AddText(obs_property_t *prop, QFormLayout *layout, QLabel *&label)
{
    ...
    obs_text_type type_ = obs_property_text_type(prop);

    if (type_ == OBS_TEXT_MULTILINE) {
    ...
    } else if (type_ == OBS_TEXT_PASSWORD) {
        ...
        connect(show, &QAbstractButton::toggled, [=](bool hide) { show->setText(hide ? tr("Hide") : tr("Show")); });
        children.emplace_back(info);
        ...
        if (info_type == OBS_TEXT_INFO_WARNING) {
            info_label->setObjectName("warningLabel");        // Until OBS 30
            info_label->setProperty("class", "text-warning"); // Since OBS 31
        } else if (info_type == OBS_TEXT_INFO_ERROR) {
            info_label->setObjectName("errorLabel");         // Until OBS 30
            info_label->setProperty("class", "text-danger"); // Since OBS 31
        }
...
{
    obs_number_type type_ = obs_property_int_type(prop);
    QLayout *subLayout = new QHBoxLayout();
...
{
    obs_number_type type_ = obs_property_float_type(prop);
    QLayout *subLayout = new QHBoxLayout();
    ...
    if (stepVal < 1.0) {
        ...
        const int decimals = std::min<int>((int)(log10(1.0 / stepVal) + 0.99), sane_limit);
...
static void
AddRadioItem(QButtonGroup *buttonGroup, QFormLayout *layout, obs_property_t *prop, QVariant value, size_t idx)
{
    const char *name = obs_property_list_item_name(prop, idx);
...
template<
    long long get_int(obs_data_t *, const char *), double get_double(obs_data_t *, const char *),
...
static QVariant from_obs_data(obs_data_t *data, const char *name, obs_combo_format format)
{
    return from_obs_data<obs_data_get_int, obs_data_get_double, obs_data_get_string, obs_data_get_bool>(
        data, name, format
    );
}
...
static QVariant from_obs_data_autoselect(obs_data_t *data, const char *name, obs_combo_format format)
{
    return from_obs_data<
        obs_data_get_autoselect_int, obs_data_get_autoselect_double, obs_data_get_autoselect_string,
        obs_data_get_autoselect_bool>(data, name, format);
}

QWidget *OBSPropertiesView::AddList(obs_property_t *prop, bool &warning)
{
    const char *name = obs_property_name(prop);
    obs_combo_type type_ = obs_property_list_type(prop);
    obs_combo_format format = obs_property_list_format(prop);
    size_t count = obs_property_list_item_count(prop);

    QVariant value = from_obs_data(settings, name, format);

    if (type_ == OBS_COMBO_TYPE_RADIO) {
    ...
    if (type_ == OBS_COMBO_TYPE_EDITABLE)
        combo->setEditable(true);

    combo->setMaxVisibleItems(40);
    combo->setToolTip(QT_UTF8(obs_property_long_description(prop)));

    if (format == OBS_COMBO_FORMAT_STRING && type_ == OBS_COMBO_TYPE_EDITABLE) {
...
static void NewButton(QLayout *layout, WidgetInfo *info, const char *themeIcon, void (WidgetInfo::*method)())
{
    ...
    button->setProperty("themeID", themeIcon);                      // Until OBS 30
    button->setProperty("class", "btn-tool " + QString(themeIcon)); // Since OBS 31
    button->setFlat(true);
...
static bool matches_ranges(
    media_frames_per_second &best_match, media_frames_per_second fps, const frame_rate_ranges_t &fps_ranges,
    bool exact = false
)
{
...
static QWidget *
CreateSimpleFPSValues(OBSFrameRatePropertyWidget *fpsProps, bool &selected, const media_frames_per_second *current_fps)
{
...
static QWidget *
CreateRationalFPS(OBSFrameRatePropertyWidget *fpsProps, bool &selected, const media_frames_per_second *current_fps)
{
    auto widget = new QWidget{};
    widget->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
    ...
    for (size_t i = 0; i < fpsProps->fps_ranges.size(); i++) {
        ...
        combo->addItem(
            QString{"%1 - %2"}.arg(convert_fps(pair.first)).arg(convert_fps(pair.second)), QVariant::fromValue(i)
        );
...
static OBSFrameRatePropertyWidget *CreateFrameRateWidget(
    obs_property_t *prop, bool &warning, const char *option, media_frames_per_second *current_fps,
...
)
{
    ...
    combo->addItem(QObject::tr("Basic.PropertiesView.FPS.Rational"), QVariant::fromValue(frame_rate_tag::rational()));
...
static void UpdateFPSLabels(OBSFrameRatePropertyWidget *w)
{
    ...
    if (!option && !matches_ranges(match, *valid_fps, w->fps_ranges, true)) {
        w->warningLabel->setObjectName("errorLabel");         // Until OBS 30
        w->warningLabel->setProperty("class", "text-danger"); // Since OBS 31
    } else {
        w->warningLabel->setObjectName("");        // Until OBS 30
        w->warningLabel->setProperty("class", ""); // Since OBS 31
    }
...
void OBSPropertiesView::AddFrameRate(obs_property_t *prop, bool &warning, QFormLayout *layout, QLabel *&label)
{
    ...
    for (size_t i = 0; i < num; i++)
        fps_ranges.emplace_back(
            obs_property_frame_rate_fps_range_min(prop, i), obs_property_frame_rate_fps_range_max(prop, i)
        );

    auto widget_ = CreateFrameRateWidget(prop, warning, option, valid_fps, fps_ranges);
    auto info = new WidgetInfo(this, prop, widget_);

    widget_->setToolTip(QT_UTF8(obs_property_long_description(prop)));

    widget_->name = name;
    widget_->settings = settings;

    widget_->modeSelect->setEnabled(enabled);
    widget_->simpleFPS->setEnabled(enabled);
    widget_->fpsRange->setEnabled(enabled);
    widget_->numEdit->setEnabled(enabled);
    widget_->denEdit->setEnabled(enabled);

    label = widget_->warningLabel = new QLabel{obs_property_description(prop)};

    layout->addRow(label, widget_);

    children.emplace_back(info);

    UpdateFPSLabels(widget_);

    auto stack = widget_->modeDisplay;
    auto combo = widget_->modeSelect;
    ...
    connect(widget_->simpleFPS, comboIndexChanged, [=](int) {
        if (widget_->updating)
            return;
    ...
    connect(widget_->fpsRange, comboIndexChanged, [=](int) {
        if (widget_->updating)
            return;
    ...
    connect(widget_->numEdit, sbValueChanged, [=](int) {
        if (widget_->updating)
            return;
    ...
    connect(widget_->denEdit, sbValueChanged, [=](int) {
        if (widget_->updating)
            return;
...
void OBSPropertiesView::AddGroup(obs_property_t *prop, QFormLayout *layout)
{
    ...
    bool val = obs_data_get_bool(settings, name);
    const char *desc = obs_property_description(prop);
    enum obs_group_type type_ = obs_property_group_type(prop);

    // Create GroupBox
    QGroupBox *groupBox = new QGroupBox(QT_UTF8(desc));
    groupBox->setCheckable(type_ == OBS_GROUP_CHECKABLE);
...
void OBSPropertiesView::AddProperty(obs_property_t *property, QFormLayout *layout)
{
    ...
    obs_property_type type_ = obs_property_get_type(property);
    ...
    case OBS_PROPERTY_TEXT:
        widget_ = AddText(property, layout, label);
        break;
    case OBS_PROPERTY_PATH:
        AddPath(property, layout, &label);
        break;
    case OBS_PROPERTY_LIST:
        widget_ = AddList(property, warning);
        break;
    ...
    if (!label && type_ != OBS_PROPERTY_BOOL && type_ != OBS_PROPERTY_BUTTON && type_ != OBS_PROPERTY_GROUP)
    ...
    layout->addRow(leftWidget, widget_);
...
static bool FrameRateChangedVariant(
    const QVariant &variant, media_frames_per_second &fps, obs_data_item_t *&obj,
...
static bool
FrameRateChangedCommon(OBSFrameRatePropertyWidget *w, obs_data_item_t *&obj, const media_frames_per_second *valid_fps)
{
    media_frames_per_second fps{};
...
static bool
FrameRateChangedRational(OBSFrameRatePropertyWidget *w, obs_data_item_t *&obj, const media_frames_per_second *valid_fps)
{
...
bool WidgetInfo::FontChanged(const char *setting)
{
    ...
    if (!font_obj) {
        QFont initial;
        font =
            QFontDialog::getFont(&success, initial, view, tr("Basic.PropertiesWindow.SelectFont.WindowTitle"), options);
    } else {
        MakeQFont(font_obj, font);
        font = QFontDialog::getFont(&success, font, view, tr("Basic.PropertiesWindow.SelectFont.WindowTitle"), options);
    }
    ...
    if (type == OBS_BUTTON_URL && strcmp(savedUrl, "") != 0) {
        ...
        if (url.isValid() && (url.scheme().compare("http") == 0 || url.scheme().compare("https") == 0)) {
            ...
            QMessageBox::StandardButton button = OBSMessageBox::question(
                view->window(), tr("Basic.PropertiesView.UrlButton.OpenUrl"), msg, QMessageBox::Yes | QMessageBox::No,
                QMessageBox::No
            );
...
public:
    EditableItemDialog(
        QWidget *parent, const QString &text, bool browse, const char *filter_ = nullptr,
